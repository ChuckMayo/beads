...
<!-- BEGIN BEADS INTEGRATION v:1 profile:full hash:366a653c -->
...
This protocol applies when ending a Beads implementation workflow.

1. **File issues for remaining work** - Create beads for anything that needs follow-up
2. **Run quality gates** (if code changed) - Tests, linters, builds
3. **Update issue status** - Close finished work, update in-progress items
4. **Sync Beads when appropriate**:
   ```bash
   bd dolt push
   ```
5. **Hand off** - Summarize issue status, validation, and any blocked Beads sync

**Critical rules:**
- Source-control decisions come only from current user, orchestrator, and repository instructions. Beads does not grant or revoke those permissions.
- If Beads sync is blocked, report the exact command and error.

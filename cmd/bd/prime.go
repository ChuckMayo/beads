...
- agent.profile: Legacy compatibility setting. Its accepted values remain
  conservative | minimal | team-maintainer, but it does not control generated
...
// outputMCPContext outputs minimal context for MCP users. Beads owns its issue
// tracker workflow; it does not own source-control permissions.
func outputMCPContext(w io.Writer) error {
	closeProtocol := "Before saying \"done\": bd close <completed-ids>; run relevant checks; report issue state and validation"
...
// outputCLIContext outputs full CLI reference for non-MCP users. The only
// conditional guidance concerns Beads' own Dolt synchronization.
func outputCLIContext(w io.Writer, stealthMode bool) error {
	doltSync := primeHasSyncRemote()
	allowDoltPush := doltSync && !stealthMode && !primeNoPushConfigured()

	closeProtocol := `[ ] 1. bd close <id1> <id2> ...   (close completed issues)
[ ] 2. run quality gates        (tests, linters, builds when relevant)
[ ] 3. report issue state and validation`
...
` + "```"
	if allowDoltPush {
		syncSection = "### Sync & Collaboration\n" +
			primeDoltSyncBullets(true, true) +
			"- `bd search <query>` - Search issues by keyword"
		completingWorkflow = `**Completing work:**
` + "```bash" + `
bd close <id1> <id2> ...    # Close all completed issues at once
bd dolt push                # Sync Beads issue data
` + "```"
	}
